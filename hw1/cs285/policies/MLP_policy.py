...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        # maybe do a Gaussian Likelihood here
        # raise NotImplementedError
        from torch.distributions.normal import Normal

        return Normal(self.mean_net(observation), self.logstd.exp())
    ...
    def update(self, observations, actions):
        ...
        observations = ptu.from_numpy(
            observations
        )  #  torch.from_numpy(observations).float().to(ptu.device)
        actions = ptu.from_numpy(actions)  #   torch.from_numpy(actions).to(ptu.device)
        self.optimizer.zero_grad()
        pdf = self.forward(observations)
        loss = -pdf.log_prob(actions).mean()
        loss.backward()
        self.optimizer.step()
        return {
