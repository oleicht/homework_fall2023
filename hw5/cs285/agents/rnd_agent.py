...
class RNDAgent(DQNAgent):
    ...
    def update_rnd(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        loss = F.mse_loss(input=self.rnd_net(obs), target=self.rnd_target_net(obs))
    ...
    def update(
        ...
        with torch.no_grad():
            ...
            rnd_error = (
                (self.rnd_net(observations) - self.rnd_net(observations)) ** 2
            ).mean(dim=-1)
            # rnd_error = (rnd_error - rnd_error.mean()) / (rnd_error.std() + 1e-6)
            assert rnd_error.shape == rewards.shape
            rewards = rewards.to(torch.float32) + self.rnd_weight * rnd_error
